...
public class Liste {
                ...
                for (Fichier fichier : noRepeat) {
                    ...
                    try {
                        ...
                        if (arrOfStr[0].length() != 5) {
                            String msg = key + "  : error in file name (length != 5 characters). \n";
                            pref_err++;
                            writer.write(msg);
                        }

                        if (arrOfStr[1].length() != 3) {
                            String msg = key + "  : error in file extension (length != 3 characters). \n ";
                            suf_err++;
                            writer.write(msg);
                        }
                        ...
                        if (!result.equals(new IntWritable(1))) {
                            String msg = key + "  is present more than one time in Lnom (exactly " + result + " times \n";
                            dupl_err++;
                            writer.write(msg);
                        }
                    }
                    catch(ArrayIndexOutOfBoundsException exception) {
                        String msg = key + "  is not a file. \n";
                        writer.write(msg);
                    }
                }
                writer.write("------------------------------------------ \n");

                /*writer.write("*** Errors in prefix: "+ pref_err+ " \n");
                writer.write("*** Errors in suffix: "+ suf_err+ " \n");
                writer.write("*** Duplication errors: "+ dupl_err+ " \n");*/
    ...
    public static void getMatches() throws SQLException, IOException {
        ...
        String first_line = "*** Research result *** \n \n";
        ...
        writer.close();
        System.out.println("________________end");
    }
